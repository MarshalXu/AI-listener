...
所有决策为 SDD-r2 proposed；Board 批准后才可实施。
...
决策理由：Qwen3-ASR 的官方 vLLM streaming 不能直接用于原生 macOS CoreML 路径；CoreML partial/final、转换成熟度、可锁定工件尺寸/内存与转换依赖许可证链证据不足，首版不进入候选基准，也不下载/转换权重。若未来有上游可复现的流式 CoreML artifact、完整许可证/SBOM，并能在固定基准机评估，须新 ADR 和 Board 重批后才可入围。不得把“未测”写成“不达标”。
