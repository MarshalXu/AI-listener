...
`relativePath` 必须在应用资产根下且不可含 `..`；`commitState ∈ temporary|stable|committed|quarantined|deleting`。`stable` 表示 rename 与目录 fsync 已完成但 SQLite 尚未确认 commit，绝不等于可展示的 ready。MVP 只允许单声道；资产与 Session 一对一。SQLite 保存相对路径，重定位容器不破坏引用。

## `C-07 RecoveryManifest`

Required：`manifestVersion, sessionId, temporaryRelativePath, stableRelativePath, phase, expectedByteCount?, expectedSha256?, updatedMonotonicMs`。

`phase ∈ temporary|stablePendingCommit|committed|quarantined`。manifest 在首帧写入前创建；每次 phase 变化均以 temp-write、文件 fsync、atomic rename、parent-directory fsync 发布。路径必须通过应用资产根 canonical containment 校验。manifest 不是业务真相源，只是跨文件系统 rename 与 SQLite commit 窗口的恢复意图；`D-11` 的联合判定优先于单字段猜测。
