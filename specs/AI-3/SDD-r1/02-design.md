...
- `D-11` 创建：先插入 `recording` Session，再创建 `<sessionId>.audio.tmp`；停止：关闭临时文件并计算 hash → 原子 rename 为 stable asset → 同一 SQLite 事务登记 AudioAsset、finalized segments、Session `ready`。若 DB commit 失败，stable 文件进入 recovery manifest，不自动删除。
- 启动扫描仅处理应用自有目录：`recording/finalizing` Session、`.tmp`、recovery manifest。可读音频恢复为 `recovered`/可播放；损坏或空文件隔离并显示可操作错误。绝不把缺失逐字稿当成音频失败。
