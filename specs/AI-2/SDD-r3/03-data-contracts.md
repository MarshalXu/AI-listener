...
`RecordingState = idle | requestingPermission | recording | stopping | succeeded | failed | unknown`。`succeeded` 必须有 microphone 与 system 两个 final artifact；`failed` 不得被下游工件消费者使用。
...
## FutureTranscriptionInput v1（仅本地交接边界，不实现）
...
`ArtifactReference` 只含 `artifactId`、`track`、`container`、`codec`、格式、frameCount、durationMs 和受控本地读取句柄/相对路径。它不得含原始音频 bytes、上传 URL、供应商/模型信息、locale、摘要或行动项。该结构仅满足 AI-2 对未来接口字段的验收要求，不定义实时 ASR、模型选择、转写结果或外部处理；当前里程碑不得实例化它。任何转写流程均需独立规格、隐私审查与 Board 审批。
