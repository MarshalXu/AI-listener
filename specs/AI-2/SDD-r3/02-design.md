...
- `succeeded`：当前会话两份 final 文件及 committed Session Manifest 已确定性提交，每份文件均可打开并解码至少一个音频 packet/frame；不表示实际播放已经发生或完成。可发起逐轨播放，也可回到 `idle` 开始新会话。
...
| `stopping` | 两路停止、关闭、可解码校验、两工件与 manifest 提交均成功，且无先前错误 | 生成两个 `AudioArtifact` 与 committed `SessionManifest` | `succeeded` |
...
6. 每个 buffer 记录来源 PTS、timescale、映射后的 session 单调纳秒 offset 与 frameCount；不得使用墙钟推导对齐。

## 共享单调时间轴

- 单位：持久化统一使用 UInt64 nanoseconds；中间换算使用整数有理数，`sourceNs = roundHalfAwayFromZero(sourcePTSValue × 1_000_000_000 / sourcePTSTimescale)`，拒绝 timescale ≤ 0、负值与溢出。
- 来源：AVFoundation 使用每个 `AVAudioTime`/sample buffer 的有效 host/sample presentation timestamp；ScreenCaptureKit 使用音频 `CMSampleBuffer` presentation timestamp。适配器必须保留来源 clock domain 标识，禁止把墙钟或回调到达时间当作 PTS。
- 原点：协调器在两路启动前采样一次系统连续单调时钟为 `sessionEpochHostTimeNs`。每路首个有效 buffer 同时保存 `sourceFirstPTS` 和同一 clock domain 下的 `firstFrameHostTimeNs`。映射式为 `sessionOffsetNs = firstFrameHostTimeNs - sessionEpochHostTimeNs + (sourceNs - sourceFirstNs)`。
- clock domain 校验：若来源 timestamp 不能经 Apple 公共时间换算 API映射到 host-time domain，启动失败 `clock.unmappable`；不得用回调到达时间猜测。
- 起始偏差：`abs(microphone.firstFrameSessionOffsetNs - system.firstFrameSessionOffsetNs)`，尖刺验收阈值 ≤ 100,000,000 ns（100 ms）。
- 漂移：在共同覆盖的 10 分钟窗口端点，分别计算“映射 PTS 增量 − 按 frameCount/sampleRate 推导的音频持续时间”；两路差值的绝对值为 `crossTrackDriftNs`，阈值 ≤ 50,000,000 ns（50 ms/10 min）。短于 10 分钟的录音只记录观测值，不宣称通过 10 分钟漂移门。
- 超阈值：起始偏差超限视为启动失败；10 分钟漂移超限经 `stopping` 进入 `failed(clock.driftExceeded)`。保留 failed manifest/诊断，不生成 succeeded。阈值变化属于实质规格变更，须重新审批。
...
- 成功校验：文件存在、大小大于容器头、可读取音频属性、frameCount > 0，并通过独立 reader 打开且解码至少一个音频 packet/frame。该校验不要求连接输出设备或完成实际播放。
...
停止顺序：状态切为 `stopping` → 禁止新 buffer 接受边界 → 请求两采集源停止 → 等待已接收 buffer 串行落盘 → 关闭两 writer → 独立 reader 可解码校验 → 写 `preparingCommit` manifest 临时文件并 fsync → microphone 文件原子改名 → manifest 记录第一步并 fsync → system 文件原子改名 → manifest 记录第二步并 fsync → 将 manifest 状态改为 `committed` 并原子替换正式 manifest → 目录同步 → 进入 `succeeded`。

若第二次改名或任一 manifest 持久化步骤失败，提交事务失败：已改名的第一份文件确定性改名为 `.abandoned`（若回滚改名也失败则保留原名但 manifest 标为 `recoveryRequired`，启动恢复时隔离），另一份 partial 同样标记 abandoned；不得存在 `committed` manifest，不得进入 `succeeded`。恢复只依据 manifest 的 `commitSteps` 与实际受控相对路径交叉检查，不猜测成功。

播放只接受 `succeeded` 会话中的 final artifact。实际播放产生独立 `PlaybackAttempt`；播放完成不是 `succeeded` 守卫。播放失败不破坏已通过可解码校验的录音成果：会话录音状态仍为 `succeeded`，另行呈现 `playback.failed` 操作错误与重试/显示位置建议。

## 崩溃恢复

| manifest/文件观察 | 确定性恢复 |
|---|---|
| 无 manifest 或 `recording/stopping`，存在 partial | 会话置 `failed`；文件改名 `.abandoned`；记录 `recovery.abandonedPartial` |
| `preparingCommit`，零份已改名 | 两份 partial → `.abandoned`；manifest → `failed` |
| `preparingCommit`，恰一份已改名 | 已改名文件与剩余 partial 均 → `.abandoned`；manifest → `failed` |
| `preparingCommit`，两份已改名但 manifest 未 committed | 两份均 → `.abandoned`；manifest → `failed`；禁止事后提升成功 |
| `committed` 且两文件存在并可解码 | 恢复为 `succeeded` |
| `committed` 但文件缺失/不可解码或路径冲突 | manifest → `recoveryRequired`/会话 `failed`；隔离现存文件，不覆盖、不删除 |
...
| 播放失败 | 录音仍 `succeeded`；PlaybackAttempt `failed` | 重试另一轨、显示位置；记录 playback error |
...
2. `succeeded` 意味着两份 final artifact 与 committed manifest 已提交，且两文件均通过独立 reader 的至少一个 packet/frame 解码检查；不代表实际播放完成。
3. 任一时刻最多一个 active session。
4. 失败后无活跃 capture/writer。
5. partial 永不被播放或报告为成功。
6. 未经用户意图与系统授权不启动采集。
7. 音频及敏感元数据不出本机。
8. 不存在 committed manifest 时，会话绝不恢复或报告为 `succeeded`。
