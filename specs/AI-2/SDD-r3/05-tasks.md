...
- 输入：完整 SDD-r3、批准 revisionId、批准目录。
...
- 输入：Design 状态转移、不变量、RecordingSession/SessionManifest/PlaybackAttempt v1。
- 输出：可编译的契约模型、串行状态机、可注入 Clock/ID/failure seams。
- 验收证据：每个合法/非法转移的单元测试；重复 start/stop、首要错误保留、两路首 buffer/clock threshold 守卫证据；schema round-trip/未知字段测试。
...
- 依赖：T-02。
- 负责人：Founding macOS Engineer。
- 输入：ADR-002/003/004、Artifact/Session/SessionManifest 契约。
- 输出：会话目录、两路 CAF writer、partial/close/decode-probe/manifest commit、失败 manifest 与 abandoned 扫描。
- 验收证据：临时容器单测；写入/关闭/两次改名/每次 manifest fsync/目录同步/磁盘不足 failure injection；第一份改名后第二份失败的确定性隔离；各崩溃点恢复矩阵；成功两工件 + committed manifest 不变量；日志无绝对路径扫描结果。
...
- 依赖：T-02 至 T-06。
- 负责人：Founding macOS Engineer。
- 输入：两采集适配器、store、完整状态机、共享单调时间轴映射。
- 输出：双路启动/回滚、recording 双指示与计时、幂等停止、成功/失败呈现。
- 验收证据：两路首 buffer 前不 recording；任一路缺失时另一条被释放；PTS 映射单测、100 ms 起始偏差边界、10 分钟 50 ms 漂移边界；`idle → requestingPermission → recording → stopping → succeeded` 实体证据；运行期中断、停止/写入失败证据。
...
- 验收证据：两轨 decode probe；两轨实际播放分别 completed；播放失败注入；失败不改变 session succeeded；partial 被拒绝的测试。
