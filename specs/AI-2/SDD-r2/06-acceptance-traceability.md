...
| V-04 | 正常录制 | 点击停止 | `recording → stopping → succeeded`；恰有两份 final CAF、committed Session Manifest；两文件各自 decode probe ≥1 packet/frame；未要求实际播放 | 状态序列、manifest revisions/commitSteps、格式/帧/字节/decode probe | P-04；不变量 2/8 |
| V-05 | V-04 工件且 session 已 succeeded | 逐一播放 microphone/system | 两个独立 PlaybackAttempt 均 `completed`；session 始终 succeeded | playback started/completed 事件、人工听检记录 | 播放能力；ADR-007 |
...
| V-12 | open/append/close/decode probe/每个 commit step 任一步失败 | 录制或停止 | failed；不得有 committed manifest；partial/final 被确定性隔离 | 每阶段 failure injection | P-06 |
| V-13 | 已 succeeded 工件，注入输出设备/播放路由失败 | 播放 | PlaybackAttempt failed；session 仍 succeeded；显示 playback error 与恢复动作 | 状态断言、错误事件 | P-06；ADR-007 |
...
| V-19 | 合成两路 PTS，含 timescale 差异、边界与不可映射 clock | 映射到 session 单调轴 | 有理数换算确定；99/100 ms 允许，>100 ms 以 clock.startSkewExceeded 失败；不用墙钟/回调时间 | 单元/属性测试、映射向量、状态事件 | P-09；ADR-002 |
| V-20 | 10 分钟合成 buffer 序列 | 注入 49/50/>50 ms 跨轨漂移 | ≤50 ms 通过；>50 ms 经 stopping 到 failed(clock.driftExceeded)；短录音只报告不宣称通过 | fake clock/buffer 测试、ClockMetrics | P-09；ADR-002 |
| V-21 | 在每个 commit step 后模拟崩溃，尤其 microphone 已改名而 system 失败 | 重启恢复 | 无场景提升为 succeeded；两文件隔离/manifest failed 或 recoveryRequired；committed 且两文件可解码才恢复 succeeded | 故障注入、目录与 manifest 前后快照 | P-04/P-06；SessionManifest |
