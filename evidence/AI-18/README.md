...
2026-07-30 本机执行 `scripts/package-release-app.sh`：Release build 成功，
`codesign --verify --deep --strict` 成功，bundle 约 58 MiB。当前逐文件清单本身
SHA-256 为 `798413f56adf8549e1f9dfa689531d75cf8b3da125d53eb4c381f1c92facc920`。
...
2026-07-30 Release 全仓测试以隔离 cache 执行：构建成功；共发现 44 个测试，
43 个通过。唯一失败是
...
| M-07 ASR 慢/失败 | 自动化已验证 | `StreamingASRTests` bounded queue、gap、writer-first、engine failure |
...
的真实播放器自动测试也没有通过。其余主要风险是 ad-hoc 签名首次运行体验与真人
TCC。自动化/Release 包定稿后，owner `local-board` 按
